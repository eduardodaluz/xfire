    ...
    extends TestCase
{
     ...
     * getClass().getResourceAsStream("sample-wsse-request.xml"); Document doc =
     * STAXUtils.read(DocumentBuilderFactory.newInstance().newDocumentBuilder(),
     * STAXUtils.createXMLStreamReader(inStream, null), false);
     * 
     * WSS4JInSecurityProcessor processor = new WSS4JInSecurityProcessor();
     * Document decrypted = null; InSecurityResult result =
     * processor.process(doc); assertEquals("cupareq",result.getUser());
     * assertEquals("cupareq1",result.getPassword()); decrypted =
     * result.getDocument(); DOMUtils.writeXml(decrypted.getDocumentElement(),
     * System.out); }
     */
    ...
    /*
     * public void testEncryption() throws Exception{ Document doc =
     * readDocument("tcpmon.xml"); PropertiesLoader a = new PropertiesLoader ();
     ...
     * (); doc = outProcessor.process(doc); DOMUtils utils = new DOMUtils ();
     * utils.writeXml(doc, System.out);
     * 
     * InSecurityDefaultBuilder builder = new InSecurityDefaultBuilder ();
     ...
     * 
     * WSS4JInSecurityProcessor processor = new WSS4JInSecurityProcessor();
     * builder.build(processor); SecurityResult result = processor.process(doc);
     * int z=0; DOMUtils.writeXml(result.getDocument(), System.out); }
     */
